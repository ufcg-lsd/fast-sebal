...
void albedo_function(double reflectance_line[][8], Sensor sensor, double tal_line[], int width_band, int number_sensor, int line, TIFF *albedo){
    ...
    for (int col = 0; col < width_band; col++){
        albedo_line[col] = reflectance_line[col][1] * sensor.parameters[1][sensor.WB] + reflectance_line[col][2] * sensor.parameters[2][sensor.WB] + reflectance_line[col][3] * sensor.parameters[3][sensor.WB] + reflectance_line[col][4] * sensor.parameters[4][sensor.WB] + reflectance_line[col][5] * sensor.parameters[5][sensor.WB] + reflectance_line[col][final_tif_calc] * sensor.parameters[final_tif_calc][sensor.WB];
...
void lai_function(double reflectance_line[][8], int width_band, int line, TIFF *lai){
    ...
    for (int col = 0; col < width_band; col++){
        ...
        savi_line[col] = ((1.05) * (reflectance_line[col][4] - reflectance_line[col][3])) /
                         ((0.05) + (reflectance_line[col][4] + reflectance_line[col][3]));
...
void enb_emissivity_function(); //so eh usada no TS
void eo_emissivity_function();
void ea_emissivity_function(); //so eh usada no RLat
