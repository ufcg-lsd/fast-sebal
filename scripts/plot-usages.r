...
proctimes = proctimes[c("SYSTEM", "ELAPSED")]
...
setnames(proctimes, c("FASE", "TIMESTAMP"))
proctimes$FASE[1] = "FASE1"
proctimes$FASE[2] = "FASE2"
proctimes$FASE[3] = "FASE3"
...
pplot = ggplot(cpu_usage, aes(x=TIMESTAMP, y=USAGE, group=TYPE, colour=TYPE)) +
	...
	annotate("text", x = med_proctimes, y=0.0, label = proctimes$FASE) + 
...
pplot = ggplot(data_disk, aes(x=TIMESTAMP, y=MB.S, color=TYPE)) + 
	...
	ylab("MB/S") + 
	geom_vline(xintercept = proctimes$TIMESTAMP, linetype=2, size=0.15) + 
	annotate("text", x = med_proctimes, y=-10.0, label = proctimes$FASE) + 
	scale_colour_manual(values=c("#FF7777", "#0066CC")) + 
	theme_bw() + 
	guides(fill=guide_legend(title=NULL)) + 
	theme(legend.title=element_blank())
